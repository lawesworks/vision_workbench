(() => {
  const CONFIG = {
    ...
    routes: [
      ...
      { label: "Image", path: "/inference-url" },
      { label: "Video", path: "/inference-video" },
      { label: "Webcam", path: "/stream/webcam?index=0" },
      { label: "API", path: "/inference-api" },
      { label: "RTSP", path: "/stream/rtsp?url=rtsp://192.168.1.160:8554/live" },
      { label: "Trainer", path: "/train-google" },
  ...
  let currentStreamType = null; // "webcam" | "rtsp" | null
  ...
  */
  ...
  async function loadRoute(path) {
    ...
    if (nextIsWebcam) currentStreamType = "webcam";
    else if (nextIsRtsp) currentStreamType = "rtsp";
    else currentStreamType = null;
  ...
  */
  ...
  iframeEl.addEventListener("load", () => {
    ...
    const isRtsp = src.includes("/stream/rtsp");
    const isWebcam = src.includes("/stream/webcam");

    if (isRtsp) {
      loaderHideTimer = setTimeout(() => {
        hideLoader();
        loaderHideTimer = null;
      }, 4000); // change to 3000-5000 as desired
    } else if (isWebcam) {
      loaderHideTimer = setTimeout(() => {
        hideLoader();
        loaderHideTimer = null;
